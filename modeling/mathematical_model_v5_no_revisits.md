...
_Note: These two constraints imply flow balance ($\sum_{in} = \sum*{out}$) and prevent any node from being visited more than once.*

### 3. Base Station Constraints

If a drone is used ($z_k=1$) it must depart from the base exactly once and return exactly once; unused drones ($z_k=0$) cannot leave the base.
...
1.  **Variables**: $x_{ij}^k$ changed from Integer to Binary $\{0,1\}$; added $z_k$ to allow unused drones.
...
3.  **Objective**: Pure minimax (tie-break penalty removed).
4.  **Base & Activation**: Base exit/entry tied to $z_k$; optional activation links keep unused drones idle.
5.  **Capacity**: Flow capacity uses $|P|$ to avoid bottlenecks when many nodes are assigned to one drone.
