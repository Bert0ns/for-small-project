...
- $Z \in \mathbb{R}_{\ge 0}$: The maximum time taken by any drone (the variable to minimize).
...
### 1. Visit Every Point Exactly Once
...
### 5. Subtour Elimination (SEC)

To prevent cycles that do not include the base, we use Subtour Elimination Constraints (SECs). Unlike the MTZ formulation, this does not require auxiliary variables ($u_i$) or Big-M constants.
Since there are exponentially many such constraints, they are typically added dynamically as __Lazy Constraints__ during the solving process (i.e., whenever the solver finds a solution with a subtour, a constraint cutting off that specific subtour is added).

$$ \sum_{i \in Q} \sum_{j \notin Q} \sum_{k \in K} x_{ijk} \ge 1 \quad \forall Q \subset P, Q \ne \emptyset $$

This states that for any proper subset of points $Q$, there must be at least one drone leaving the set $Q$.

### 6. Symmetry Breaking (Optional)

Since the drones are identical, any permutation of the tours among the drones results in an equivalent solution. To reduce the search space and improve performance, we can enforce an ordering on the tours, for example, by requiring the travel times to be non-increasing:

$$ \sum_{(i,j) \in A} t_{ij} x_{ijk} \ge \sum_{(i,j) \in A} t_{ij} x_{ij(k+1)} \quad \forall k \in \{1, \dots, K-1\} $$
